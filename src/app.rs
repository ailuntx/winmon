...
impl App {
  pub fn new() -> WithError<Self> {
    ...
    Ok(Self { cfg, device, ..Default::default() })
  }

  fn update_metrics(&mut self, data: Metrics) {
    self.e_cpu_usage.push(data.e_cpu_usage.0 as u64, data.e_cpu_usage.1 as f64);
    self.p_cpu_usage.push(data.p_cpu_usage.0 as u64, data.p_cpu_usage.1 as f64);
    self.gpu_usage.push(data.gpu_usage.0 as u64, data.gpu_usage.1 as f64);
  ...
  fn render_usage_block(&self, f: &mut Frame, area: Rect, label: &str, store: &UsageStore) {
    let label = if store.top_value > 0 {
      format!("{label} {:3.0}% @ {:4} MHz", store.usage * 100.0, store.top_value)
    } else {
  ...
  fn power_title(&self, label: &str, store: &PowerStore, temp: Option<f32>) -> (String, String) {
    let left = if store.available {
      format!("{label} {:.2}W ({:.2}, {:.2})", store.top_value, store.avg_value, store.max_value)
    } else {
      format!("{label} N/A")
    };
  ...
  fn render(&mut self, f: &mut Frame) {
    let title = format!(
      "{} | {}C/{}T | {}",
      self.device.machine_name, self.device.cpu_cores, self.device.cpu_threads, self.device.gpu_name
    );
