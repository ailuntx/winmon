...
fn bootstrap_runtime_assets_windows() -> WithError<()> {
  ...
  let stable_ohm = stable_dir.join("third_party").join("ohm").join("OpenHardwareMonitor_x64.exe");
...
fn winmon_stable_dir() -> Option<std::path::PathBuf> {
  std::env::var("APPDATA").ok().map(|appdata| std::path::PathBuf::from(appdata).join("winmon"))
}
...
"#
  ...
  if output.status.success() {
    ...
    let msg = if stderr.is_empty() { "更新用户 PATH 失败".to_string() } else { stderr };
    Err(msg.into())
  }
}
...
#[cfg(windows)]
fn run_powershell_json_with_env<T: DeserializeOwned>(script: &str, envs: &[(&str, String)]) -> WithError<T> {
  let command = format!(
  ...
  if !output.status.success() {
    let stderr = String::from_utf8_lossy(&output.stderr).trim().to_string();
    let msg = if stderr.is_empty() { "PowerShell 执行失败".to_string() } else { stderr };
    return Err(msg.into());
  }
