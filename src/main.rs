...
fn pipe_mode(interval: u32, samples: u32, include_device: bool) -> WithError<()> {
  ...
  let device = if include_device { Some(sampler.get_device_info().clone()) } else { None };
...
fn main() -> WithError<()> {
  ...
  match args.command {
    Some(Commands::Pipe { samples, device_info }) => pipe_mode(args.interval, samples, device_info),
